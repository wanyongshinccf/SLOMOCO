function qa_slomoco_v22(TR,tdim,zdim, dx, dy, dz, vol_filename, sli_filename, tfile)
...
outofplane6=zeros(zmbdim*tdim,6);
inplane6=zeros(zmbdim*tdim,6);
...
% construct input mesh, inputdata
exclude_slices_one=[];
exclude_slices_two=[];
    
if MB == 1
    % identify outer-most slices according to slice timing
    if (slice_timing==1 || slice_timing==3)
        endslices_two=[1 ceil(zdim/2) ceil(zdim/2)+1 zdim];
        % if even # of slices, need to alter this
        endslices_one=[1 ceil(zdim/2)];
    elseif (slice_timing==2)
        endslices_two=[1 2 zdim-1 zdim];
        endslices_one=[1 zdim];
    else
        disp('unsupported slice timing number, choose 1 (alt, siemens style) or 2 (seq)');
    end

    for i=1:length(outofplane6)/zdim
        exclude_slices_one=[exclude_slices_one [((zdim*(i-1))+endslices_one)]];
        exclude_slices_two=[exclude_slices_two [((zdim*(i-1))+endslices_two)]];
    end
end
...
% if (slice_timing==1)
%   % for Siemens scanners only, even #s of slices are acquired starting on second slice
%   if (mod(zdim,2))
%     %inverse_slice_order=[1 17 2 18 3 19 4 20 5 21 6 22 7 23 8 24 9 25 10 26 11 27 12 28 13 29 14 30 15 31 16];
%     slice_order=[[1:2:zdim] [2:2:zdim]];
%   else
%     slice_order=[[2:2:zdim] [1:2:zdim]];
%   end
% elseif (slice_timing==2)
%   % pure sequential ascending order
%   slice_order=[1:zdim];
% elseif (slice_timing==3)
%   % interleaved ascending order
%   slice_order=[[1:2:zdim] [2:2:zdim]];
% end
% slice_pos=((slice_order-0.5)*dz-dz*zdim/2);
slice_pos=((uniq_acq_order-0.5)*dz-dz*zdim/2);
...
fp=fopen('slomoco.TDmetric.ver2.txt','w'); fprintf(fp,'%g\n',td_slomoco); fclose(fp);
fp=fopen('slomoco.TDzmetric.ver2.txt','w'); fprintf(fp,'%g\n',td_slomocoz); fclose(fp);
% for a volumetric metric of motion corruption, use the max across slices within a volume
fp=fopen('slomoco.volumetric.TDzmetric.ver2.txt','w'); fprintf(fp,'%g\n',max(reshape(td_slomocoz,[zmbdim tdim]))); fclose(fp);
fp=fopen('slomoco.volumetric.TDmetric.ver2.txt','w'); fprintf(fp,'%g\n',max(reshape(td_slomoco,[zmbdim tdim]))); fclose(fp);
...
% for i=1:6
%   volmotion(:,i)=reshape(repmat(motion(:,i)',[zdim 1]),[zdim*tdim 1]);
% end
...
% pacemotion(zdim+1:end,:)=volmotion(1:end-zdim,:);
% 
% 
...
% fp=fopen('slomoco.combined.reg12dof.txt','w'); fprintf(fp,'%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\n',combined12dof'); fclose(fp);
 ...
 saveas(gcf,'qa_slomoco_metrics_ver2.jpg');
 ...
 saveas(gcf,'qa_slomoco_motionvectors_ver2.jpg');
